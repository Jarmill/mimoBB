function [x_final, S_final, c_final, run_log] = BB_1d(A, b, tau, delta, norm_type, w)
...
if (ischar(norm_type) && strcmp(norm_type, 'tv'))
    total_variation = 1;
    
    %reweighted heuristic for the weights
    if nargin < 6
        w0 = 1e-5;
    else
        w0 = w;
    ...
    A0 = A;
    A = cumsum(A, 2, 'reverse'); %equivalent to computing AS, where S are
                                 %the atoms of the systems (rows of 1)
    norm_type = 1;                             
else
    total_variation = 0;
end
...
while ~terminate  
    ...
    if isempty(S_bag)
        ...
        grad_new = A'*(Ax_new-b) + delta*x_new;
