function [atom_bag_list, N_added] = bag_owl_svd(grad, x, w, N_bag, S, tau, on_boundary)
...
%replace later for iterative things
[U, S, V] = svd(W, 'econ');
    ...
    if DG > DG_min
        if N_bag > 1
            ...
            if isempty(U)
                ...
                [Up, Sp, Vp] = addblock_svd_update(U, S, V, w_dir, 0);
            end
