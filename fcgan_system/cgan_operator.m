function [x, as, hist, iter] = cgan_operator(data, y,param)
...
%
%%%%%%%%%%%%
% Modifications of code by Marina Vinyes and Guillaume Obozinski, 2016
% %%%%%%%%%%%%
...
if isnumeric(data)
    ...
    %Marina's code uses a design/data matrix D rather than an operator A. A
    %here means atoms
    D0 = data;
    Dt = @(r) D0'*r;
    D  = @(x) D0*x;
...
as.atoms=sparse([],[],[],p,max_nb_atoms,max_nb_atoms);
as.DA=sparse([],[],[],n,max_nb_atoms,max_nb_atoms);
...
%     [maxval,new_atom]=feval(param.lmo,-g,param);
    ...
    %Jared note: My code's LMO_1d has atom first, then polar norm of
    %-gradient
%     [new_atom, maxval]=feval(param.lmo,-g,param);
    [new_atom, maxval]=param.lmo(-g)
    
    new_atom_ind = find(new_atom);
    maxval=rho*maxval;
    prev_atom = new_atom_ind;
    if maxval>0,
        A=1:atom_count;
