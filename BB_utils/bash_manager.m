...
%             obj.orig_index = 0;
        ...
        function  obj = add_atoms(obj, S_bag, AS_bag)
            ...
            if obj.atom_count + N_bag >= obj.atom_size_max
                S_pad = sparse(size(obj.S, 1), 200);
                AS_pad = sparse(size(obj.AS, 1), 200);
                
                obj.S = [obj.S S_pad];
                obj.AS = [obj.AS AS_pad];
                obj.atom_size_max = obj.atom_size_max + 200;
...
%             end
        ...
        function [obj]= ext_to_full(obj, y)
                ...
                while ~inner_done
                    ...
                    if stable_face
                        if ~obj.update_ext.exterior
                            ...
                            if all(sign(grad_full) == -1)  || all(abs(grad_full) < 1e-10)                      
                                %stable-optimal face
                                inner_done = 1;
                            else
